...
function launcher_assets() {
    // Get PAGE Template
    $launcher_page = basename(get_page_template());
    if(is_page()){
        if($launcher_page == "launcher.php"){
            ...
            // DATA Transfer to main.js
            $arfa_year = get_post_meta(get_the_ID(),"year",true);
            $arfa_month = get_post_meta(get_the_ID(),"month",true);
            $arfa_day = get_post_meta(get_the_ID(),"day",true);
            // wp_localize_script
            wp_localize_script("main-jquery-js","cfdata",array(
                "year"  => $arfa_year,
                "month" => $arfa_month,
                "day"   => $arfa_day,
            ));
